...
function gfeedfetcher(divid, divClass, linktarget){
    ...
    this.feedsfetched=5 //number of feeds fetched
    this.feedlimit=10
...
gfeedfetcher.prototype.addFeednull=function(label, url){
    this.feedlabels[this.feedlabels.length]=label
    this.feedurls[this.feedurls.length]=url
}
...
gfeedfetcher.prototype.displayoptions=function(parts){
    this.showoptions=("date, snippet, label, description") //set RSS entry options to show ("date, datetime, time, snippet, label, description")
}
...
gfeedfetcher.prototype.init=function(){
    ...
    this.feedcontainer.innerHTML='<p><img src="'+gfeedfetcher_loading_image+'"</p>'
    var displayer=this
...
gfeedfetcher.prototype._fetch_data_as_array=function(result, ddlabel){  
    ...
    if (thisfeed==""){ //if error has occured fetching feed
        var gfeedfetcher_loading_image=""
    } else {
...
gfeedfetcher.prototype._displayresult=function(feeds){
    ...
    for (var i=0; i<feeds.length; i++){
        ...
        if (feeds[i].link.length == 0) {
            var itemtitle="<span class=\"titlefield\">" + feeds[i].title + "</span>"
            var itemlabel=/label/i.test(this.showoptions)? '<span class="labelfield">'+this.feeds[i].ddlabel+'</span>' : " "
            var itemdate=gfeedfetcher._formatdate(feeds[i].publishedDate, this.showoptions)
            var itemdescription=/description/i.test(this.showoptions)? '<span class="langlabel">'+feeds[i].content : /snippet/i.test(this.showoptions)? " "+feeds[i].contentSnippet+'</span>'  : ""
            rssoutput+=this.itemcontainer + itemtitle + " " + itemdescription + " " + itemlabel + "\n" + this.itemcontainer.replace("<", "</") + "\n\n"
        } else {
            ...
            rssoutput+=this.itemcontainer + itemtitle + " " + itemdescription + " " + itemlabel + " " + itemdate + "\n" + this.itemcontainer.replace("<", "</") + "\n\n"
        }
    }
